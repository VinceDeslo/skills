...
| [clone-org](skills/clone-org/SKILL.md) | Clone every non-archived repository of a GitHub organization that is not yet checked out under a local root, previewed as a table for approval before anything is cloned |
...
| [entity-machine](skills/entity-machine/SKILL.md) | Crawl codebases for a named entity and render a self-contained HTML page of its state machine — every state with its meaning, one or more diagrams with every transition edge keyed and coloured by actor, and a table per diagram citing trigger, guard, event, and code location |
...
| [generate-artifact](skills/generate-artifact/SKILL.md) | Build a self-contained HTML artifact on any subject into `~/.artifacts`, and serve that directory locally behind a generated index that lists every artifact and picks up new files on reload |
...
| [pr-assignments](skills/pr-assignments/SKILL.md) | Crawl every open GitHub PR whose review was requested from you in the last two weeks, summarize each in a line with its comment conversation, CI state, and review status, and rank them by review-SLA expiry — direct requests first, then team requests, each badged with CI, peer-review, and your own review state — into a self-contained HTML report delivered as an artifact through `generate-artifact` |
| [pr-digest](skills/pr-digest/SKILL.md) | Review a PR from its link in four in-session passes — orient, correctness, fit, verify — and emit a short HTML digest of at most five major, five minor, and five nits as an artifact through `generate-artifact`, without touching the PR |
