---
name: refresh-work
description: Fast-forward the default branch of every work repository under ~/repos/work so local clones match their remotes. Use when asked to refresh, update, or pull the latest work repos, the work or infracost projects, or when the user says "refresh-work".
compatibility: Requires bash, git, a checkout root at ~/repos/work, and push/fetch credentials for each remote (SSH keys or a credential helper).
---

# refresh-work

Walk every repository under `~/repos/work`, fetch its remote, and fast-forward the default branch.
