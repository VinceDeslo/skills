...
refresh_root() {
    local root="${1%/}"
    local candidate listing fields repo_key default_branch target has_head remote ahead behind before dirty
    local repo_id name location suffix pull_error after
...
echo
echo "updated=$updated up-to-date=$current skipped=$skipped failed=$((failed + missing))"
[ "$((failed + missing))" -eq 0 ]
