...
description: Fast-forward the default branch of every personal repository under ~/repos/personal so local clones match their remotes, using worktrunk (wt) to discover repositories and worktrees. Use when asked to refresh, update, or pull the latest personal repos, the personal projects, or when the user says "refresh-personal".
compatibility: Requires bash, worktrunk (`wt`, https://worktrunk.dev), jq, git, a checkout root at ~/repos/personal, and fetch credentials for each remote (SSH keys or a credential helper).
---

# refresh-personal

Walk every repository under `~/repos/personal` and fast-forward its default branch. Worktrunk is the source of truth for repository and worktree structure.
